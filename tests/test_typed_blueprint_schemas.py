...
def test_llm_schema_requires_explicit_file_binding() -> None:
    ...
    assert any("binding" in error for error in _errors(document, "llm-interface.schema.json"))
...
def test_machine_schema_accepts_python_and_command_file_bindings_only() -> None:
    ...
    assert _errors(python_document, "machine-interface.schema.json") == []
    assert _errors(command_document, "machine-interface.schema.json") == []
    assert _errors(inline_document, "machine-interface.schema.json")
...
def test_machine_bindings_reject_parent_traversal() -> None:
    ...
    for binding in [
        ...
        assert _errors({**base, "binding": binding}, "machine-interface.schema.json")
