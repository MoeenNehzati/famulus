...
def test_editable_candidate_requires_explicit_repo_root(tmp_path):
    with pytest.raises(ManagedRuntimeError, match="repo_root is required for editable mode"):
        build_candidate_release(
            runtime_root=tmp_path / "runtime",
            manifest_path=REAL_MANIFEST,
            platform="linux",
...
def test_explicit_standard_context_rejects_editable_true(tmp_path):
    repo = _launcher_repo(tmp_path)
    context = _launcher_context(tmp_path, repo, mode="standard")
    manifest = tmp_path / "runtime_dependencies.json"
    manifest.write_text('{"version": 2, "skills": {}}')

    with pytest.raises(ManagedRuntimeError, match="editable does not match installation context"):
        build_candidate_release(
...
def test_explicit_development_context_rejects_editable_false(tmp_path):
    repo = _launcher_repo(tmp_path)
...
def test_development_candidate_points_to_exact_live_launcher_resources(monkeypatch, tmp_path):
    ...
    install_calls = [call for call in calls if call[1:3] == ["pip", "install"]]
    assert install_calls[1][-4:] == [
        "--no-deps",
        "--no-build-isolation",
        "--editable",
        str(repo.resolve()),
...
# famulus-skip: category=capability-unavailable; reason=requires the pinned real uv binary on PATH; alternate=mocked development-context coverage proves the editable command and context-derived default without external downloads
@pytest.mark.skipif(not PINNED_UV_AVAILABLE, reason="pinned uv is not on PATH")
def test_development_candidate_real_uv_resolves_officina_from_checkout(tmp_path):
    selected_home = tmp_path / "development-home"
    ...
    assert result.returncode == 0, result.stderr
    resolved_module = Path(result.stdout.strip()).resolve()
    assert resolved_module.is_relative_to((REPO_ROOT / "src" / "officina").resolve())
