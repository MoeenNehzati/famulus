...
def make_typed_skill(repo: Path, name: str = "demo-skill") -> Path:
    ...
    (repo / "skills" / "skill-certifier").mkdir(parents=True, exist_ok=True)
    return skill
...
def test_healthy_child_is_reused_without_git_check(
    ...
    assert child_path.is_file(), [outcome.as_payload() for outcome in outcomes]
...
def test_typed_certification_writes_authenticated_graph_and_pooled_health(
    ...
    key = (tmp_path / "skills" / "skill-certifier" / ".health-authentication-key").read_bytes()
...
def test_dirty_manifest_declared_policy_document_blocks_all_new_stamps(tmp_path: Path) -> None:
    skill = make_typed_skill(tmp_path)
    make_commit_backed(tmp_path)
    policy_document = tmp_path / "docs" / "certification_and_drift.md"
...
def test_missing_target_schema_finishes_semantics_without_key_or_stamp(tmp_path: Path) -> None:
    ...
    key_path = tmp_path / "skills" / "skill-certifier" / ".health-authentication-key"
...
def test_symlinked_key_parent_is_a_not_written_readiness_reason(tmp_path: Path) -> None:
    skill = make_typed_skill(tmp_path)
    make_commit_backed(tmp_path)
    key_root = tmp_path / "skills" / "skill-certifier"
...
def test_skill_certifier_contract_declares_runtime_io_and_partial_results() -> None:
    skill_root = MODULE_PATH.parents[1]
    source = yaml.safe_load(
        (
            skill_root / "blueprints" / "rtx-audit-certifier.yaml"
    ...
    read_contents = {
        item["content"] for item in contract["direct_io"]["reads"]
    }
    write_contents = {
        item["content"] for item in contract["direct_io"]["writes"]
    }
    assert "cryptography" not in {
        dependency["name"] for dependency in source["runtime_dependencies"]
    ...
    assert "cryptography" in {
        dependency["name"]
        for dependency in runtime_manifest["skills"]["skill-certifier"][
            "interfaces"
        ]["certify"]["dependencies"]
        if dependency["kind"] == "python-package"
    }
    ...
    assert "independently" in contract["execution"]["consistency"]["per_source"]
...
def test_public_audit_target_routes_v4_through_certificate_writer(
    tmp_path: Path,
    monkeypatch,
) -> None:
    ...
    def certify_v4(repo_root: Path, **kwargs: object):
    ...
    monkeypatch.setattr(certifier, "_certify_v4_repository", certify_v4)

    outcome, evidence = certifier._audit_target(
        FakeDispatcher(tmp_path),
        target_for(tmp_path, skill_root),
        (),
    ...
    owned = tuple(
        sorted(
            ...
            if node.skill_root == skill_root
        )
    )
    assert evidence == ()
    assert len(calls) == 1
    assert calls[0]["repo_root"] == tmp_path.resolve()
    assert calls[0]["target_node_ids"] == owned
    assert calls[0]["reviewed_commit"] == commit
    assert calls[0]["require_migration_review"] is False
    assert outcome.semantic_status == "passed"
    assert {node.node_id for node in outcome.nodes} == set(owned)


def test_public_v4_audit_requires_explicit_repository_bound_review(
    tmp_path: Path,
    monkeypatch,
) -> None:
    ...
    missing, _evidence = certifier._audit_target(
        FakeDispatcher(tmp_path),
        target_for(tmp_path, skill_root),
        (),
        "2026-07-23T12:00:00-04:00",
    )
    wrong_root, _evidence = certifier._audit_target(
        FakeDispatcher(tmp_path),
        target_for(tmp_path, skill_root),
        (),
        "2026-07-23T12:00:00-04:00",
        reviewed_repository=tmp_path / "other",
        reviewed_commit=commit,
    )

    assert missing.semantic_status == "failed"
    assert wrong_root.semantic_status == "failed"
    assert any(
        "explicit LLM-reviewed repository" in reason
        for node in missing.nodes
        for reason in node.reasons
    )
    assert any(
        "does not match the target repository" in reason
        for node in wrong_root.nodes
        for reason in node.reasons
    )
    assert writes == []


def test_v4_mechanical_gate_excludes_ordinary_test_suite(
    tmp_path: Path,
    monkeypatch,
) -> None:
    commands: list[list[str]] = []

    def run_local(name: str, command: list[str], **kwargs: object):
        commands.append(command)
        return certifier.CommandResult(name, command, 0, "", "")

    monkeypatch.setattr(certifier, "run_local_command", run_local)

    results = certifier.run_v4_mechanical_checks(
        FakeDispatcher(tmp_path),
        repo_root=tmp_path,
    )
    ...
    assert commands == [[sys.executable, "validators/runner.py"]]


def test_public_v4_certification_rejects_multi_repository_attestation(
    tmp_path: Path,
    monkeypatch,
) -> None:
    first_root = tmp_path / "first"
    second_root = tmp_path / "second"
    _first_graph, _first_states, first_commit = _v4_fixture(first_root)
    _v4_fixture(second_root)
    resolved = {
        "first": target_for(
            first_root,
            first_root / "skills" / "demo-skill",
        ),
        "second": target_for(
            second_root,
            second_root / "skills" / "demo-skill",
        ),
    }
    monkeypatch.setattr(
        certifier,
        "collect_exact_target",
        lambda _dispatcher, request: resolved[request],
    )

    with pytest.raises(
        certifier.AuditError,
        match="may attest only one repository",
    ):
        certifier.certify(
            FakeDispatcher(first_root),
            targets=["first", "second"],
            skip_mechanical=True,
            reviewed_repository=first_root,
            reviewed_commit=first_commit,
        )
...
def test_v4_candidate_owned_certifier_accepts_its_temporary_repository(
    tmp_path: Path,
    ...
    source_path = graph.nodes["demo-skill.source.gateway"].blueprint_path
    source = yaml.safe_load(source_path.read_text(encoding="utf-8"))
    source["description"] = "Reviewed by the candidate-owned certifier."
    _write_v4_yaml(source_path, source)
    reviewed_commit = _commit_v4_review(tmp_path, "candidate-owned semantic review")
    monkeypatch.setattr(certifier, "REPO_ROOT", tmp_path)
...
def test_live_v4_writer_does_not_require_migration_review_metadata(
    tmp_path: Path,
    monkeypatch,
) -> None:
    graph, _states, commit = _v4_fixture(tmp_path)
    ...
    monkeypatch.setattr(certifier, "blueprint_v4_mechanical_commit", fail_if_read)
...
def test_private_v4_writer_aborts_pre_append_races(tmp_path: Path, race: str) -> None:
    ...
    def mutate(node_id: str) -> None:
        if race == "tracked-input":
            ...
            (tmp_path / "unexpected.txt").write_text("race\n", encoding="utf-8")
        else:
...
def test_private_v4_writer_derives_repository_only_at_batch_boundaries(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    graph, _states, _commit = _v4_fixture(tmp_path)
...
def test_private_v4_writer_runs_full_readiness_only_at_batch_boundaries(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    graph, _states, _commit = _v4_fixture(tmp_path)
