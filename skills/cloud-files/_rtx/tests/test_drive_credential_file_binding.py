"""Drive credential-file binding tests."""
...
def ensure_oauth():
    return _load_module("cloud_files_credential_file_ensure", "_ensure_oauth.py")


@pytest.fixture(scope="module")
def drive_gateway():
    return _load_module("cloud_files_credential_file_gateway", "_drive_gateway.py")
    ...
    def add(
        ...
        *,
        ...
        descriptors[resolved] = SimpleNamespace(
            ...
            subject=subject,
            granted_scopes=(
                google_credentials.SERVICE_SCOPES["drive"]
                if scoped
...
def test_bind_normalizes_probes_then_preserves_config(
    ...
    assert requests[0].full_url.endswith("/files?pageSize=1&fields=files(id)")
    assert requests[0].headers["Authorization"] == "Bearer drive-access-token"
...
def test_malformed_and_unreadable_config_are_terminal(
    ensure_oauth,
    drive_gateway,
    google_file_fakes,
    monkeypatch: pytest.MonkeyPatch,
    invalid_contents: bytes,
) -> None:
    home = google_file_fakes.home
    descriptor = google_file_fakes.add(home / "credentials" / "drive.json")
    config_path = home / ".config" / "cloud-files" / "config.json"
    ...
    with pytest.raises(drive_gateway.CloudFilesError, match="config"):
        drive_gateway.load_config(home)
...
def test_present_non_file_config_is_terminal(
    ensure_oauth,
    drive_gateway,
    google_file_fakes,
    kind: str,
...
def test_present_invalid_file_binding_is_terminal(
    drive_gateway,
    tmp_path: Path,
    value,
) -> None:
    ...
    with pytest.raises(Exception, match="credential_file"):
        drive_gateway.load_config(tmp_path)
...
def test_runtime_prefers_file_over_id_and_legacy(
    ...
    descriptor = (tmp_path / "drive.json").resolve()
    config = drive_gateway.CloudFilesConfig(
        remote_llm_root="assistant/",
        timeout_seconds=45,
        credentials_path=tmp_path / "legacy.json",
        credential_id="legacy-id",
        credential_file=descriptor,
        home=tmp_path,
    )
    ...
    monkeypatch.setattr(
        ...
        lambda *_args, **_kwargs: pytest.fail("ID fallback must not run"),
    )
    monkeypatch.setattr(
        drive_gateway,
        "load_credentials",
    ...
    assert drive_gateway.get_access_token(config) == "fresh-token"
