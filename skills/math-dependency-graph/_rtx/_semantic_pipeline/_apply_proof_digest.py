#!/usr/bin/env python3
"""Apply a proof digest to produce strict proof-free semantic IR."""
...
def _require_statement_location(value: object, label: str) -> list[int]:
    if (
...
def _validate_semantic_ir(payload: object) -> dict:
    record = _require_keys(
        payload,
        {"ir_version", "files", "document", "inventory", "entities", "exclusions", "unresolved_resolutions", "relationships", "hint_decisions", "reference_decisions", "gap_decisions", "gaps"},
        {"ir_version", "files", "document", "inventory", "entities", "exclusions", "unresolved_resolutions", "relationships", "hint_decisions", "reference_decisions", "gap_decisions", "gaps", "edgeless_justification"},
        "transitional semantic IR",
    )
    if record["ir_version"] != 2:
        raise ValueError("transitional semantic IR has unsupported version")
    if (
        not isinstance(record["files"], list)
        or not record["files"]
        or len(record["files"]) != len(set(record["files"]))
        or not all(isinstance(item, str) and item for item in record["files"])
    ):
        raise ValueError("transitional semantic IR files must be a unique nonempty array")
    ...
    for index, entity in enumerate(record["entities"]):
        entity_record = _require_keys(
            entity,
            {"candidate_ids", "id", "type", "statement_location", "short_title", "description", "source"},
            {"candidate_ids", "id", "type", "kind", "category_label", "statement_location", "short_title", "title", "description", "source", "ref"},
        ...
        _require_statement_location(
            entity_record["statement_location"],
            f"semantic entity {index}.statement_location",
        )
        if entity_record["statement_location"][0] >= len(record["files"]):
            raise ValueError(
                f"semantic entity {index}.statement_location file index is invalid"
            )
...
def _validate_provenance_report(report: object) -> None:
    ...
    for index, entity in enumerate(record["proof_entities"]):
        item = _require_keys(
            entity,
            {"candidate_ids", "id", "type", "kind", "statement_location", "short_title", "description", "source"},
            {
                "candidate_ids", "id", "type", "kind", "category_label", "short_title",
                "title", "statement_location", "description", "source", "ref",
            },
        ...
        _require_statement_location(
            item["statement_location"],
            f"proof report entity {index}.statement_location",
        )
