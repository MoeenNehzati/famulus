#!/usr/bin/env python3
"""Compile resolved mathematical semantics into canonical visualization JSON.
...
def compile_semantic_graph(
    ...
    located_entities.sort(
        key=lambda item: (
            _source_position(item[1], inventory["files"]),
            item[0]["id"],
        )
    )
...
class Interface(PythonArgvMachineInterface):
    ...
    prog = "semantic_graph_compiler.py"
