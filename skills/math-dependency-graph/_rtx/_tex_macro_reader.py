...
# A LaTeX package command and its MathJax equivalent, where one command is a
# drop-in for the other at the same arity. MathJax implements a subset of LaTeX,
# so a macro body reaching into a package renders as literal text with no error
# from any layer. Extend this table when a package command has a genuine MathJax
# counterpart; commands with no counterpart are reported instead, never guessed.
PACKAGE_COMMAND_EQUIVALENTS: dict[str, str] = {
    "bm": "boldsymbol",          # bm.sty -> MathJax boldsymbol extension
    "bold": "boldsymbol",        # older amsbsy spelling
    "hs": "hspace",
    "vs": "vspace",
}

# MathJax ships these TeX extensions, and their names match the LaTeX packages
# they emulate. A `\usepackage` naming one of them is safe.
MATHJAX_EXTENSIONS: frozenset[str] = frozenset({
    "action", "ams", "amscd", "bbox", "boldsymbol", "braket", "bussproofs",
    "cancel", "cases", "centernot", "color", "colortbl", "empheq", "enclose",
    "extpfeil", "html", "mathtools", "mhchem", "newcommand", "noerrors",
    "noundefined", "physics", "require", "setoptions", "tagformat", "textcomp",
    "textmacros", "unicode", "upgreek", "verb", "amsmath", "amssymb", "amsfonts",
})

USEPACKAGE_RE = re.compile(r"\\usepackage\s*(?:\[[^\]]*\])?\s*\{([^}]*)\}")


def normalize_package_commands(macros: dict[str, object]) -> tuple[dict[str, object], dict[str, int]]:
    """Rewrite package commands inside macro bodies to their MathJax equivalents."""
    applied: dict[str, int] = {}
    if not PACKAGE_COMMAND_EQUIVALENTS:
        return macros, applied
    pattern = re.compile(
        r"\\(" + "|".join(sorted(PACKAGE_COMMAND_EQUIVALENTS, key=len, reverse=True)) + r")(?![A-Za-z])"
    )
    ...
    for name, body in macros.items():
        if isinstance(body, list) and len(body) == 2:
            out[name] = [body[0], rewrite(str(body[1]))]
        else:
            out[name] = rewrite(str(body))
    return out, applied


def unsupported_packages(entrypoint: Path) -> list[str]:
    r"""Return `\usepackage` names MathJax neither implements nor has an equivalent for."""
    try:
        text = flatten_tex(entrypoint)
    except Exception:  # pragma: no cover - reporting must never break extraction
        return []
    names: list[str] = []
...
def main(argv: Iterable[str] | None = None) -> None:
    ...
    macros, rewritten = normalize_package_commands(macros)
        ...
        json.dumps(
            {
                ...
                "out": str(out_path),
                "macros": len(macros),
                "rewritten_package_commands": rewritten,
