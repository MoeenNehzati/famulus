...
8. Report the semantic draft and canonical paths, the optional HTML and serving address, whether the canonical artifact is render-ready, browser-diagnostics status only when checked, label resolution, embedded macro count, represented scope, and unresolved extraction gaps.
