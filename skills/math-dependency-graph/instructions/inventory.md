...
1. Invoke `math-dependency-graph._rtx.interface.scripts-next-inventory-unit` with the supplied state directory and worker index, without an acknowledgement, to obtain the first unit. Measure the actual dispatcher call at its caller boundary and retain only its nonnegative process-startup and total elapsed milliseconds; never estimate or fabricate either value.
...
5. Acknowledge that exact unit by invoking the same `next` interface with its returned unit id. Add `--wrap` when this unit closes the consecutive attention sequence for its owning object or result. The acknowledgement validates your current inventory before it advances and returns either the next unit or `complete`.
6. Repeat from step 2 for every returned unit, measuring every actual next dispatcher call in the same way. Finish only when a successful acknowledgement returns state `complete`.
