...
1. Invoke `math-dependency-graph._rtx.interface.scripts-next-inventory-unit` using exactly `<state-dir> <worker-index> [--ack <unit-id>] [--wrap] [--retry-code validation-failed]`: supply the state directory and worker index without an acknowledgement to obtain the first unit. The iterator state already binds your inventory and progress paths and the inventory schema. Those paths are your file responsibilities, never arguments to `next`. Retain the response's nonnegative `process_timings_ms.process_dispatch` and `process_timings_ms.total` measurements verbatim; they cover the public wrapper's controlled child spawn through bootstrap and complete child stdout/exit, not outer gateway latency. Use `--retry-code validation-failed` only when correcting a completed fragment after the controller's public pooling attempt rejected this exact worker and durably authorized its retry; preserve the final acknowledgement's original `--wrap` choice. A bare retry claim, another worker's authorization, an unchanged rejected artifact, or a changed wrap is refused.
