...
- What source-owned interfaces does it expose or use, and how are instruction
  routes selected?
...
### 3. Interface and instruction-source decomposition audit
Use the characterization to check whether the skill has multiple substantial
instruction use cases, distinct read-only and mutating paths, or other plausible
source or interface-boundary problems. If it does, read
`references/skill-standards/interface-design.md` and identify:
- Whether the `SKILL.md` gateway source should keep the full workflow or become
  a router plus shared parent policy.
- Which use cases, if any, need separate read-only, mutating,
  provider-specific, or staged instruction sources.
- Which behavior belongs in each source, which source-owned interfaces it
  declares, which behavior is shared policy, and which reference files each
  route should load.
- Whether the module and source blueprints plus generated blocks match the
  source, interface, export, and gateway-file layout.

If none of those signals is present, record that the gateway instruction source
and its interfaces remain cohesive and continue without loading the guide.
