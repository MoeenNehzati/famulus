...
if __package__ and __package__.count('.') >= 1:
    from .._schedule_backend._linux_backend import LinuxScheduleBackend, cron_to_systemd_calendar
else:
...
if __package__ and __package__.count('.') >= 1:
    from .._schedule_backend._windows_backend import WindowsScheduleBackend, cron_to_schtasks_args, task_name
else:
...
def test_linux_sync_writes_units_and_enables_timer(tmp_path):
    ...
    assert f'"{SKILL_DIR / "_job_executor.py"}" --jobs-file' in service
    assert "_rtx/_rtx" not in service.replace("\\", "/")
...
def test_osx_sync_writes_plist_and_loads_launch_agent(tmp_path):
    ...
    assert plist["ProgramArguments"][1] == str(SKILL_DIR / "_job_executor.py")
    assert "_rtx/_rtx" not in plist["ProgramArguments"][1].replace("\\", "/")
...
def test_windows_sync_creates_task_scheduler_entry():
    ...
    assert str(SKILL_DIR / "_job_executor.py") in command
    assert "_rtx/_rtx" not in command.replace("\\", "/")
