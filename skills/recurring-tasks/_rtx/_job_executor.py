...
def run_job(*, jobs_file: Path, job_name: str, log_dir: Path = LOG_DIR) -> int:
    ...
    # Tell the launch chain where this job's inputs live, rather than leaving
    # each layer to rediscover them.
    #
    # A scheduled run reaches an agent CLI through invoke-skill and the agent
    # launcher, which between them need the agent definition, the profile, and
    # the hooks. Those all ship in one tree, so naming the tree is enough --
    # and only this process reliably knows it, resolving it from its own
    # location, which is correct in a dev checkout, a plugin cache, and a
    # relocated repo alike. The alternatives are worse: baking absolute paths
    # in at install time freezes them, and probing the filesystem guesses.
    #
    # It has to be done here because a systemd user service inherits almost no
    # environment (PATH and the D-Bus address, in practice), so neither is set
    # even though an interactive shell has them. setdefault so an explicit
    # caller still wins.
    #
