       ...
       └─ scripts/run-skill.sh <name>
            └─ reads $AI_AGENT_COMMAND_TEMPLATE from systemd user environment
                 └─ scripts/invoke-agent.sh <name>
                      └─ sources scripts/env.sh (sets PATH; generated by installer)
...
### Key dependency: `scripts/env.sh`

`invoke-agent.sh` sources this generated file to put the assistant binary on
PATH before calling `exec assistant`. Without it, `assistant: not found` is
logged by every job. It is written by `install-assistant-tools` (the
`write_runner_env` step) and is gitignored. If it disappears, re-run the
...
scripts/
  env.sh               # generated by installer (gitignored); exports bin dir PATH for invoke-agent.sh
...
| `scripts/invoke-agent.sh <name>` | Default agent invoker: sources `scripts/env.sh` to set PATH, then calls `assistant --permission-mode bypassPermissions -p "/<name>"`. This is what `AI_AGENT_COMMAND_TEMPLATE` points to. |
| `scripts/env.sh` | **Generated by installer — do not edit.** Exports the assistant bin dir to PATH. Sourced by `invoke-agent.sh`. Regenerate by re-running `install-assistant-tools`. |
...
| All jobs log "assistant: not found" | `scripts/env.sh` missing or bin dir changed | Re-run `install-assistant-tools` to regenerate `env.sh` |
