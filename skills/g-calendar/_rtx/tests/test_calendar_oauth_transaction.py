...
MODULE_PATH = Path(__file__).resolve().parents[1] / "_oauth_bootstrap.py"
SRC_ROOT = MODULE_PATH.parents[3] / "src"
if str(SRC_ROOT) not in sys.path:
    sys.path.insert(0, str(SRC_ROOT))
SPEC = importlib.util.spec_from_file_location("g_calendar_oauth_transaction", MODULE_PATH)
...
def test_calendar_verification_uses_bearer_request(monkeypatch: pytest.MonkeyPatch) -> None:
    seen = {}
