...
def test_invalid_utf8_client_fails_before_listener_with_client_invalid(
    ...
    assert (failure.value.phase, failure.value.code) == ("client", "client_invalid")
    assert json.loads(diagnostics.getvalue())["code"] == "client_invalid"
...
def test_custom_client_secret_reference_survives_authorize_and_refresh(
    ...
    loaded = gc.load_credential_file(Path(result.credential_file))
    access_token = gc.refresh_access_token_from_file(
        Path(result.credential_file),
...
def test_authorize_services_never_calls_real_network(
    ...
    # Browser launch is supplied as the in-process callback fixture because
    # production browser launch is intentionally isolated in another process
    # and cannot inherit this monkeypatch.
    diagnostics = io.StringIO()
    with pytest.raises(authorize_services_module.AuthorizationFailure) as failure:
        authorize_services(
            ["drive"],
...
def test_no_open_browser_cli_disables_helper_and_keeps_success_stdout(
    ...
    def fake_authorize(services, **kwargs):
        ...
        return authorize_services_module.AuthorizationResult(
            ...
            subject="sub1",
            credential_file="/tmp/google-credential.json",
