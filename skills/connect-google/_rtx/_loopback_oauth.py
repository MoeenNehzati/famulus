...
and stores only the service scopes actually granted.  Browser launch runs in
...
def authorize_services(
    ...
    *,
    ...
    try:
        ...
        except GoogleCredentialPublicationUncertain as exc:
            raise AuthorizationFailure(
                phase="credential_publish", code="publication_uncertain"
            ) from exc
        except GoogleCredentialError as exc:
            raise AuthorizationFailure(
        ...
        _emit_diagnostic(
            ...
            warnings=list(ref.publication_warnings),
        )
