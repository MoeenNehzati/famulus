...
- `rutter.interface.dispenser@6` — Construct one VoyageDispenser from self-describing modes, unique run initialization, optionally prefix-filtered ID enumeration, Voyage resolution, and Voyage-or-run release callbacks.
...
dispenser binding, returned operating contract, and its assigned `voyage_id`.
...
Each Voyage agent follows the dispenser's returned operating contract using
only its assigned ID. It stops on terminal, fault, uncertain, malformed, or
unknown status.
