...
def test_development_apply_wires_editable_candidate_from_context(
    ...
    monkeypatch.setattr(
        install,
        "diagnose_installation",
        lambda **kw: install.DiagnosticReport.healthy_for(context),
    )
    ...
    assert status == 0
    assert candidate_calls[0]["installation_context"] is context
    assert candidate_calls[0]["editable"] is True
