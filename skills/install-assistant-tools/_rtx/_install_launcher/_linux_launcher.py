...
def _unix_invoke_skill_content() -> str:
    """Render the launcher the scheduler uses to start one skill.

    Runs the `background_run` agent, never `assistant`. That agent exists so an
    unattended run is configured as one: agents/background_run.md carries the
    rules that only apply when nobody can answer a question, and its Codex
    profile and Claude settings carry the model, reasoning budget, and hooks
    that suit unattended work. Routing through `assistant` meant scheduled jobs
    silently inherited an interactive profile tuned for cheap, quick replies.

    Nothing about the contract appears here on purpose: the agent definition
    delivers it, so this launcher stays a thin backend switch and the contract
    cannot drift between the two backends.
    """
    return (
        "#!/usr/bin/env python3\n"
        "# Generated by install-assistant-tools - do not edit manually.\n"
        "\"\"\"Invoke a skill as the background_run agent, on the selected backend.\"\"\"\n"
        "import os\n"
        "import sys\n"
        "\n"
        "if len(sys.argv) != 2:\n"
        ...
        "if backend == 'claude':\n"
        "    command = [\n"
        "        'background_run', '--local', '--claude', '--permission-mode',\n"
        "        'bypassPermissions', '-p', f'/{skill}',\n"
        "    ]\n"
        "elif backend == 'codex':\n"
        "    command = [\n"
        "        'background_run', '--local', '--codex', 'exec', '--skip-git-repo-check',\n"
