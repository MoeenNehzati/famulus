...
def _unix_invoke_skill_content() -> str:
    return (
        ...
        "\n"
        "def _repo_root():\n"
        "    \"\"\"Locate the repo holding agents/. $AI when the caller set it (the\n"
        "    job executor does), else follow the assistant launcher, which is a\n"
        "    symlink back into the repo in a dev install.\"\"\"\n"
        ...
        "            # Match the file, not an `agents/` directory: the launcher\n"
        "            # lives under skills/install-assistant-tools, which has an\n"
        "            # unrelated agents/ of its own and would otherwise win.\n"
        "            if (parent / 'agents' / 'background_run.md').is_file():\n"
        "                return parent\n"
        "    return None\n"
        "\n"
        "\n"
        ...
        "    unattended. The text lives in agents/background_run.md and is read\n"
        "    at launch, so editing that file changes the next run with no\n"
        "    reinstall. A missing file warns rather than failing the run: losing\n"
        "    the contract degrades a run's judgement, but refusing to start\n"
        "    guarantees the job does nothing at all.\n"
        "    \"\"\"\n"
        "    root = _repo_root()\n"
        "    path = root / 'agents' / 'background_run.md' if root else None\n"
        "    try:\n"
