...
LLM_WAKEUP_RUN_CLIENT_TESTS=1 \
  ...
  skills/llm-wakeup/_rtx/tests/test_client_integration.py
```
...
| Interface | Purpose |
| --- | --- |
| `llm-wakeup._rtx.interface.auto-policy` | Enable, disable, or inspect per-session automatic scheduling |
| `llm-wakeup._rtx.interface.infer-schedule` | Infer provider, session, and reset before scheduling |
| `llm-wakeup._rtx.interface.explicit-schedule` | Schedule with an explicit provider, session, and reset |
...
```yaml
uses_interfaces:
- interface: llm-wakeup._rtx.interface.explicit-schedule
  version: 1
```
