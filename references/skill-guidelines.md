...
For **transient data** (scratch files, intermediate pipeline outputs, data consumed within a single invocation and not needed afterwards), use `../../tmp/<skill-name>/` relative to the skill directory — the `tmp/` directory at the repository root, namespaced by skill name. This shared tmp root can be gitignored once at the repo level; it is never committed. Create the directory at runtime with `mkdir -p`.
