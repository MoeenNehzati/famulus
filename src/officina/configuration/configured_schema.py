...
by ``schema.json``. Configuration supplies values, never schema
...
def _configuration_schema_path() -> Path:
    return Path(resources.files("officina.configuration").joinpath("schema.json"))
...
def _annotation_schema() -> dict[str, Any]:
    ...
    schema = _parse_mapping(
        text,
        name="officina.configuration/schema.json",
        suffix=".json",
    )
