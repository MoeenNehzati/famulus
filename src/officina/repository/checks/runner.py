...
import officina.validators.snapshot as _validator_snapshot
...
def pytest_configure(config: pytest.Config) -> None:
    ...
    officina.validators.snapshot._selected_validator_paths:
      why:
        transforms: "Resolves canonical validator IDs against the selected view."
    officina.validators.snapshot._load_staged_paths:
      why:
...
def _pytest_phase_command(
    ...
    *,
    ...
    plugin_args = (
        ["-p", "officina.repository.checks.runner"]
        if task_id in {"combined", "validators"}
        else []
    )
    return [
...
def _capture_working_staged_paths(repo_root: Path) -> tuple[str, ...]:
    ...
    Wraps
    -----
    officina.validators.snapshot.capture_staged_paths -> preprocess: pass the repository root unchanged; postprocess: return captured paths unchanged; fixed_arguments: none
    """
...
def run_suite(
    ...
    *,
    ...
    InstantiationsFromRepo
    ----------------------
    .officina.validators.snapshot.staged_repository_view:
      why:
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    InstantiationsFromRepo
    ----------------------
    .officina.validators.snapshot._write_tracked_result:
      why:
