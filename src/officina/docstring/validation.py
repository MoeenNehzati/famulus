...
The validator layer reads normalized rules from ``policy`` and applies
...
from .policy import DocstringSchema, load_docstring_schema
...
def _collect_invalid_edges(lines: Iterable[str], section_name: str) -> tuple[ParserIssue, ...]:
    ...
    CallsFromRepo
    -------------
    .parser.validate_edge_expression:
      why:
...
def _collect_invalid_wraps(lines: Iterable[str]) -> tuple[ParserIssue, ...]:
    """Collect malformed ``Wraps`` entries. Parse logic is intentionally delegated to ``parser`` so this module remains focused on classification and issue emission.
    ...
    """
    issues: list[ParserIssue] = []
    from .parser import _parse_wrap_entry
...
def _collect_invalid_module_dependencies(
    ...
    *,
    ...
    """
    issues: list[ParserIssue] = []
    from .parser import _parse_module_dependency_section
...
def _collect_invalid_dispatch_dependencies(
    ...
    *,
    ...
    """
    issues: list[ParserIssue] = []
    from .parser import _parse_dispatch_dependency_section
...
def _collect_invalid_resources(
    ...
    *,
    ...
    """
    issues: list[ParserIssue] = []
    from .parser import _parse_resource_section
...
def _collect_invalid_dataflows(
    ...
    *,
    ...
    """
    issues: list[ParserIssue] = []
    from .parser import _parse_dataflow_section
...
def _collect_invalid_ownership(
    ...
    *,
    ...
    CallsFromRepo
    -------------
    .parser.parse_ownership_reference:
      why:
