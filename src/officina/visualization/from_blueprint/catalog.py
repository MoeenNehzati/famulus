...
RELATION_SEMANTICS = {
    "transformations": {"node_omission": {"rules": [
        {"id": "hidden-dependency-to-source", "causes": ["user-hidden"], "left_types": ["depends-on-source", "uses-interface"], "right_types": ["depends-on-source"], "outcomes": [{"type": "depends-on-source", "fidelity": "exact"}]},
        {"id": "hidden-dependency-to-interface", "causes": ["user-hidden"], "left_types": ["depends-on-source", "uses-interface"], "right_types": ["uses-interface"], "outcomes": [{"type": "uses-interface", "fidelity": "exact"}]},
        {"id": "hidden-interface-binding", "causes": ["user-hidden"], "left_types": ["uses-interface"], "right_types": ["binds-interface"], "outcomes": [{"type": "uses-interface", "fidelity": "exact"}, {"type": "dependency", "fidelity": "degraded"}]},
        {"id": "hidden-binding-layer", "causes": ["user-hidden"], "left_types": ["binds-interface"], "right_types": ["binds-interface"], "outcomes": [{"type": "binds-interface", "fidelity": "exact"}]},
        {"id": "hidden-dependency-through-other-alias", "causes": ["user-hidden"], "left_types": ["depends-on-source", "helper-dependency", "certificate-indirectly-depends"], "right_types": ["binds-interface", "facades-child-export", "facades-implementing-source", "exposes-child-interface"], "outcomes": [{"type": "dependency", "fidelity": "degraded"}]},
        {"id": "hidden-use-through-other-alias", "causes": ["user-hidden"], "left_types": ["uses-interface"], "right_types": ["facades-child-export", "facades-implementing-source", "exposes-child-interface"], "outcomes": [{"type": "dependency", "fidelity": "degraded"}]},
        {"id": "hidden-alias-to-dependency", "causes": ["user-hidden"], "left_types": ["binds-interface", "facades-child-export", "facades-implementing-source", "exposes-child-interface"], "right_types": ["depends-on-source", "uses-interface", "helper-dependency", "certificate-indirectly-depends"], "outcomes": [{"type": "dependency", "fidelity": "degraded"}]},
        {"id": "continue-hidden-coarse-dependency", "causes": ["user-hidden"], "left_types": ["dependency"], "right_types": ["depends-on-source", "uses-interface", "binds-interface", "facades-child-export", "facades-implementing-source", "exposes-child-interface", "helper-dependency", "certificate-indirectly-depends"], "outcomes": [{"type": "dependency", "fidelity": "degraded"}]},
    ]}},
    "subsumptions": [
        {"stronger_type": "depends-on-source", "weaker_types": ["dependency"]},
        {"stronger_type": "uses-interface", "weaker_types": ["dependency"]},
        {"stronger_type": "helper-dependency", "weaker_types": ["dependency"]},
        {"stronger_type": "certificate-indirectly-depends", "weaker_types": ["dependency"]},
    ],
}
...
def build_edge_categories(
    entities: Iterable[dict[str, Any]],
    *,
    derived_types: Iterable[str] = (),
) -> list[dict[str, Any]]:
    ...
    for item in result:
        if item["id"] == "routes-module":
            item.update(
                label="Routes module",
                description="Routes a namespace through a child or terminal module.",
            )
    derived = {"dependency": (
        "Dependency",
        "A dependency-bearing path whose more specific relation kind cannot be retained.",
    )}
    present = {item["id"] for item in result}
    reachable_derived_types = set(derived_types)
    result.extend(
        {"id": edge_type, "label": label, "description": description}
        for edge_type, (label, description) in derived.items()
        if edge_type in reachable_derived_types and edge_type not in present
    )
    return result
...
__all__ = ["DETAIL_LEVELS", "EDGE_PRESENTATION", "EDGE_STYLES", "RELATION_SEMANTICS", "build_edge_categories", "build_node_categories", "build_relation_semantics", "category_id"]
