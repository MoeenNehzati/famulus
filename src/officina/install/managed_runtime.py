...
def _fsync_tree(root: Path) -> None:
    """Fsync every real directory below ``root`` and its parent, bottom-up."""
    if os.name != "posix":
        return
    directories = [root]
...
def _directory_digest(root: Path) -> str:
    """Hash immutable release source while excluding generated bytecode caches."""
    digest = hashlib.sha256()
    for path in sorted(root.rglob("*")):
        if path.is_symlink():
            raise ManagedRuntimeError(f"candidate source contains a symlink: {path}")
        relative_path = path.relative_to(root)
        if "__pycache__" in relative_path.parts:
            if path.is_dir() or path.is_file():
                continue
            raise ManagedRuntimeError(f"candidate source contains a special file: {path}")
        if path.is_dir():
            continue
        if not path.is_file():
            raise ManagedRuntimeError(f"candidate source contains a special file: {path}")
    ...
    *,
    ...
    try:
        ...
        if build_wheel:
            ...
            installed_source = (
                _venv_site_packages(
                    venv_dir, platform=platform, python_version=python_version
                )
                / "officina"
            )
            atomic_files.atomic_replace_bytes(
                ...
                json.dumps(
                    {
                        ...
                        "release_id": release_id,
                        "prepared_at_ns": prepared_at_ns,
                        "source_sha256": _directory_digest(installed_source),
                    },
                    indent=2,
                ).encode("utf-8"),
                allowed_root=release_dir,
...
def _validate_release_artifact(release_dir: Path, release_id: str) -> int:
    """Validate required candidate artifact metadata and return preparation time."""
    artifact_path = release_dir / "artifact.json"
    try:
        artifact = json.loads(
            atomic_files.read_regular_file_bytes(
                artifact_path, allowed_root=release_dir
            )
        )
        if (
            ...
            or not isinstance(artifact.get("prepared_at_ns"), int)
            or artifact["prepared_at_ns"] < 0
        ):
            raise ManagedRuntimeError("prepared release artifact identity is invalid")
        kind = artifact.get("kind")
        if kind == "wheel":
            wheel = release_dir / "artifacts" / artifact["wheel"]
            wheel_bytes = atomic_files.read_regular_file_bytes(
                wheel, allowed_root=release_dir
            )
        ...
        elif kind == "source-copy":
            matches = tuple((release_dir / "venv").glob("**/site-packages/officina"))
            if len(matches) != 1 or _directory_digest(matches[0]) != artifact.get(
                "source_sha256"
            ):
                raise ManagedRuntimeError("prepared release source digest is invalid")
        else:
            raise ManagedRuntimeError("prepared release artifact kind is invalid")
        return artifact["prepared_at_ns"]
    except ManagedRuntimeError:
        raise
...
def _read_bundle_trusted_roots(runtime_root: Path, bundle_id: str) -> tuple[Path, ...]:
    ...
    try:
        roots = json.loads(
            atomic_files.read_regular_file_bytes(
                bundle / "trusted-roots.json", allowed_root=bundle
            )
        )
        return tuple(Path(root).resolve() for root in roots)
    except (OSError, UnicodeError, ValueError, TypeError) as exc:
        raise ManagedRuntimeError(f"resolver bundle trust data is invalid: {exc}") from exc


def _validate_prepared_release(runtime_root: Path, prepared: PreparedRelease) -> int:
    releases_root = (runtime_root / "releases").absolute()
    expected_release = releases_root / prepared.release_id
    release_dir = prepared.release_dir.absolute()
    if (
        _RELEASE_ID_RE.fullmatch(prepared.release_id) is None
        or release_dir != expected_release
        or prepared.release_dir.is_symlink()
        or not prepared.release_dir.is_dir()
        or releases_root.resolve() != releases_root
        or release_dir.resolve() != release_dir
    ):
        raise ManagedRuntimeError(
            "prepared release must be an exact non-symlink immediate child matching release_id"
        )
    python_bin = prepared.python_bin.absolute()
    try:
        relative_python = python_bin.relative_to(release_dir)
        prepared.python_bin.parent.resolve().relative_to(release_dir)
    except ValueError as exc:
        raise ManagedRuntimeError("prepared python_bin must live under its release") from exc
    if relative_python.parts not in {
        ("venv", "bin", "python"),
        ("venv", "Scripts", "python.exe"),
    } or not prepared.python_bin.exists():
        raise ManagedRuntimeError("prepared release interpreter identity is invalid")
    prepared_at_ns = _validate_release_artifact(release_dir, prepared.release_id)
    bundle_roots = _read_bundle_trusted_roots(
        runtime_root, prepared.resolver_bundle_id
    )
    prepared_roots = tuple(root.resolve() for root in prepared.trusted_interpreter_roots)
    if prepared_roots != bundle_roots:
        raise ManagedRuntimeError(
            "prepared trusted interpreter roots do not match resolver bundle trust data"
        )
    _validate_candidate_runtime(python_bin=prepared.python_bin)
    return prepared_at_ns


def _write_activation_metadata(
    runtime_root: Path, prepared: PreparedRelease, prepared_at_ns: int
) -> None:
    atomic_files.atomic_replace_bytes(
        prepared.release_dir / "activation.json",
        json.dumps(
            {
                "schema_version": 1,
        ...
        allowed_root=prepared.release_dir,
        mode=0o600,
    )
    _fsync_tree(prepared.release_dir)
...
def activate_prepared_release(
    ...
    prepared_at_ns = _validate_prepared_release(runtime_root, prepared)
    try:
    ...
    _write_activation_metadata(runtime_root, prepared, prepared_at_ns)
    return pointer


def _bundle_trusted_roots(runtime_root: Path, bundle_id: str) -> tuple[Path, ...]:
    return _read_bundle_trusted_roots(runtime_root, bundle_id)


def _validated_activation_order(
    runtime_root: Path,
    release_dir: Path,
    *,
    ...
    releases_root = (runtime_root / "releases").absolute()
    exact_release = release_dir.absolute()
    if (
        _RELEASE_ID_RE.fullmatch(release_dir.name) is None
        or exact_release.parent != releases_root
        or release_dir.is_symlink()
        or not release_dir.is_dir()
        or releases_root.resolve() != releases_root
        or exact_release.resolve() != exact_release
    ):
        return None
    try:
        prepared_at_ns = _validate_release_artifact(release_dir, release_dir.name)
        activation = json.loads(
            atomic_files.read_regular_file_bytes(
                release_dir / "activation.json", allowed_root=release_dir
            )
        )
        if (
            not isinstance(activation, dict)
            or activation.get("schema_version") != 1
            or activation.get("release_id") != release_dir.name
            or activation.get("prepared_at_ns") != prepared_at_ns
            or not isinstance(activation.get("activated_at_ns"), int)
            or activation["activated_at_ns"] < 0
            or (
                expected_bundle_id is not None
                and activation.get("resolver_bundle_id") != expected_bundle_id
            )
        ):
            return None
        _validate_resolver_bundle(
            runtime_root=runtime_root,
            resolver_bundle_id=activation["resolver_bundle_id"],
        )
        return activation["activated_at_ns"], prepared_at_ns
    except (ManagedRuntimeError, OSError, UnicodeError, ValueError, TypeError, KeyError):
        return None
...
def prune_old_releases(runtime_root: Path) -> tuple[Path, ...]:
    ...
    try:
        ...
        releases_root = runtime_root / "releases"
        if (
            _RELEASE_ID_RE.fullmatch(pointer.release_id) is None
            or pointer.runtime_source.absolute()
            != releases_root.absolute() / pointer.release_id
            or pointer.runtime_source.is_symlink()
        ):
            raise ManagedRuntimeError("current pointer release_id does not bind runtime_source")
        if _validated_activation_order(
            runtime_root,
            pointer.runtime_source,
            expected_bundle_id=pointer.resolver_bundle_id,
        ) is None:
            raise ManagedRuntimeError(
                "current release does not have valid successful activation metadata"
            )
        validated = []
        for path in releases_root.iterdir():
            if path.name == pointer.release_id:
                continue
            order = _validated_activation_order(runtime_root, path)
            if order is not None:
                validated.append((order, path))
        validated.sort(key=lambda item: item[0])
        previous = validated[-1][1] if validated else None
        retained = {pointer.release_id}
        if previous is not None:
            retained.add(previous.name)
        deletion_targets = [
            (order, path) for order, path in validated if path.name not in retained
        ]
        for expected_order, path in deletion_targets:
            if _validated_activation_order(runtime_root, path) != expected_order:
                raise ManagedRuntimeError(
                    "release candidate changed before pruning; no releases were removed"
                )
        removed: list[Path] = []
        for _order, path in deletion_targets:
