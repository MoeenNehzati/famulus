#!/usr/bin/env python3
"""Render an interactive dependency graph from canonical JSON.

This script does not infer domain-specific structure from source documents. The JSON input is
...
<body>
  ...
  <div class="layout" id="layout">
    ...
    <h1 id="panel-title">Dependency graph</h1>
        ...
        <div class="sidebar-section" draggable="true" data-section-id="cheatsheet">
          ...
          <details id="cheatsheet-details">
            ...
            <div class="small" style="margin-top:0.25rem;">Click a node or edge type in the legend to hide or show that type. Causality is preserved across hidden nodes via bridge edges.</div>
    ...
    const fallbackShapes = {json.dumps(CATEGORY_SHAPES, indent=2)};
    const fallbackColors = {json.dumps(CATEGORY_PALETTE, indent=2)};
    ...
    let categoryPaletteIndex = 0;
    presentCategoryIds.forEach(categoryId => {{
      const catalog = typeStyleCatalog.get(categoryId) || {{}};
      const fallbackStyle = {{
        shape: fallbackShapes[categoryPaletteIndex % fallbackShapes.length],
        color: fallbackColors[categoryPaletteIndex % fallbackColors.length],
      }};
      const style = {{
        ...fallbackStyle,
        ...catalog,
    ...
    let elk = null;
    const viewerStateKey = `dependency-graph::${{docData.document?.source_file || docData.document?.title || "document"}}`;
    ...
    function isHiddenNode(nodeId) {{
      ...
      return hiddenTypes.has(category) || hiddenNodes.has(nodeId);
    }}
    ...
    function enforceContainmentLayout(visibleEntities) {{
      ...
      childrenByContainer.forEach((childIds, containerId) => {{
        const containerPos = lastNodePositions.get(containerId);
        if (!containerPos) {{
          return;
        }}
        const sortedChildren = Array.from(new Set(childIds))
          .sort()
          .map((childId) => ({{
            id: childId,
          }}));
        ...
        const containerInnerW = Math.max(1, (containerPos.width || 220) - 2 * X_PAD);
        const columns = Math.max(
          1,
          Math.min(
            3,
            Math.max(1, Math.floor((containerInnerW + COL_GAP) / (CHILDBOX_W + COL_GAP)))
          )
        );
        ...
        sortedChildren.forEach((child, index) => {{
          ...
          let childPos = lastNodePositions.get(id);
          if (!childPos) {{
            childPos = {{
              x: containerPos.x,
              y: containerPos.y,
            ...
            lastNodePositions.set(id, childPos);
          }}
          childPos.x = containerPos.x + X_PAD + column * (CHILDBOX_W + COL_GAP);
          childPos.y = containerPos.y + HEADER_H + Y_PAD + row * (CHILDBOX_H + ROW_GAP);
          childPos.width = CHILDBOX_W;
          childPos.height = CHILDBOX_H;
        }});

        const usedCols = Math.min(columns, sortedChildren.length);
        const requiredRows = Math.ceil(sortedChildren.length / usedCols);
        const requiredWidth = X_PAD * 2 + usedCols * CHILDBOX_W + Math.max(0, usedCols - 1) * COL_GAP;
        const requiredHeight = Y_PAD * 2 + HEADER_H + requiredRows * CHILDBOX_H + Math.max(0, requiredRows - 1) * ROW_GAP;

        if (requiredWidth > (containerPos.width || 0)) {{
          containerPos.width = Math.max((containerPos.width || 0), requiredWidth);
        }}
        if (requiredHeight > (containerPos.height || 0)) {{
          containerPos.height = Math.max((containerPos.height || 0), requiredHeight);
        }}
      }});
    }}
    ...
    function renderNode(entity, position) {{
      ...
      const isContainer = isContainerNode(entity.id);
    ...
    function bindNodeInteractions(nodeEl, entity) {{
      ...
      nodeEl.addEventListener("mousedown", event => {{
        ...
        draggingNodeIds.forEach((nodeId) => {{
          ...
          const childNodeEl = nodeLayer.querySelector(`[data-node-id="${{nodeId}}"]`);
    ...
    async function updateVisibilityFull() {{
      ...
      try {{
        ...
        const graph = await computeLayout(allEntities, visibleEdges);
        ...
        const renderedNodeCount = nodeLayer.querySelectorAll(".graph-node").length;
