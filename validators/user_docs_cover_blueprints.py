...
def skill_catalog(repo_root: Path) -> list[SkillInfo]:
    ...
    Intent
    ------
    Supply shared schema-validated skill metadata to user-document checks.
...
def _validate_domain_coverage(
    ...
    """Return findings for live domains without user-document mappings.

    Intent
    ------
    Check that every non-contributor domain has a configured user-doc block.

    Rationale
    ---------
    Live blueprint domains must remain discoverable through generated user docs.

    Pseudocode
    ----------
    - if catalog and documentation root are both absent:
      - return no findings
    - set covered_domains = user-document coverage block domains
...
def _validate_user_document(
    ...
    """Return missing, malformed, or stale findings for one user document.

    Intent
    ------
    Compare one checked-in user document with its catalog-derived rendering.
...
def test_user_document(
    ...
    """Check one user document against blocks rendered from the shared catalog.
    ...
    Rationale
    ---------
    Parametrized items identify the exact stale or malformed user document.
    ...
    Wraps
    -----
    ._validate_user_document -> preprocess: none; postprocess: none; fixed_arguments: none
    """
...
def validate(repo_root: Path) -> list[str]:
    """Run user-document contracts for direct non-pytest callers.
    ...
    - for user_document in configured user documents:
      - set errors = errors plus user document findings
    - return errors
