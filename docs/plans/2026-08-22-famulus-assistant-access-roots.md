...
> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking. Follow TDD and stop commit-ready; do not commit without explicit approval.
...
- [ ] **Step 7: Implement the minimal recoverable transaction.** Record target, created-file fact, pre/post identities, ownership data, and mode before replacement. Replace only when current bytes match the preimage; an intervening edit preserves both config and pending intent. Atomically write plus flush/fsync and directory sync where supported. Recovery settles only a proven pre-state or intended post-state. Delete a created file only when verified removal leaves it empty.
...
- [ ] **Step 4: Add pinned client qualification.** Use `actions/setup-node@v4` with Node 22, `@openai/codex@0.149.0`, and `@anthropic-ai/claude-code@2.1.237`. Require the pinned `codex --version` on Linux and run allowed/denied probes through `codex sandbox` without `--add-dir` or bypass flags; label only that Linux result `host-enforcement`. macOS/Windows remain `config` plus `OS-write` until a native harness exists.
- [ ] **Step 5: Scope Claude and IDE evidence honestly.** Run `claude doctor` on all runners as `client-install-health` only. Gate authenticated three-OS Claude probes behind `FAMULUS_RUN_CLAUDE_ACCESS_SMOKE=1`. Treat skipped probes as no evidence. Treat Codex VS Code/app inheritance as documented upstream behavior, not tested enforcement, and unverified until a dedicated extension harness exists; do not use the CLI sandbox result as their proof.
- [ ] **Step 6: Route tests and artifacts explicitly.** Add focused tests explicitly to unified `tests:shared` selectors rather than relying on `tests:install` or skip-prone plugin/lifecycle tests and upload `.repo-checks/assistant-access-<os>.json` separately.
