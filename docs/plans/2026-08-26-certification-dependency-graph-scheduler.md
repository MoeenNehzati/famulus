...
**Implementation status:** Implemented on `feat/certification-audit-pool` and
adapted to the current `node-certify` and `node-drift` module names. The
authoritative blueprints, generated contract blocks, runtime dependency
manifest, and focused tests are current.
...
`node-drift` and the certification scheduler import the same decoder/validator.

## 2. Drift output

Bump the exact-repository interface to `node-drift._rtx.interface.drift-status@4` and add:
...
Create `node-certify._rtx.interface.semantic-audit-scheduler@1`:
...
`PREFIX` is confined below `skills/node-certify/_build/semantic-audit-runs/`. The scheduler owns `<PREFIX>.dag.json`, `<PREFIX>.state.json`, `<PREFIX>.inputs/`, and `<PREFIX>.lock`.

The atomic state contains:

```json
{
  "schema_version": "node-certify.semantic-audit-state/v1",
  "repository": "/absolute/resolved/repository/path",
  "dag_digest": "sha256:...",
...
`node-certify.semantic-audit-result/v1` JSON object and no surrounding prose.
```
...
Replace `node-certify/SKILL.md` steps 2–4 with:
...
- `node-drift` contains no `audited` or `in_progress` state.
