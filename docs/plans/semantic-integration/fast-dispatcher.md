...
### Source commit closure map

| Source commit | Integration commits | Focused evidence |
|---|---|---|
| `446bf71` | `0901a75`, `8cb8781`, `dc96600`, `d5584e7`, `ce1ea32`, `484c0d5` | S2-S4 and S7 progress rows; direct, certification, runtime, and checkpoint-hook results |
| `643762f` | `3ae2a40` | S8 canonical-documentation scan and checkpoint hook |
| `edf4aa5` | `57a4c63` | S1 configuration tests and checkpoint hook |
| `8fd2e97` | `0901a75`, `8cb8781` | S2 graph, schema, validator, certifier, and drift results |
| `1aa8f9e` | `dc96600` | S3 exact-lookup and forbidden-operation tests |
...
| `S1` | `E01`, `E02`; additive `T01`/`T07` configuration preservation | 50 focused repository-configuration, v6-schema, and configuration-consumer tests passed | `57a4c63` | complete |
| `S2a` | Explicit v6 graph, inventory, and shared authorization support; live v5 defaults preserved | 128 focused v6 topology plus complete graph, inventory, and authorization tests passed | `0901a75` | complete |
| `S2b` | Explicit v6 support in target-native validators, blueprint authoring/sync, certification, and drift; live v5 defaults preserved | 46 validator/tooling tests, 12 blueprint-maker tests, 57 certifier tests, and 20 drift tests passed; v6 migration schemas made self-contained for authoring | `8cb8781` | complete |
| `S3` | Exact v6 module/source lookup, hop-local namespace authorization, terminal export authority, and direct process-binding compilation without repository inventory or writes | 104 focused direct lookup, authorization, typed-error, compiler, and shared-authorization tests passed | `dc96600` | complete |
| `S4` | Explicit-config v6 host routing, confined Python execution, validated runtime-pointer binding, and dependency-free launcher injection; implicit v5 routing preserved | Direct runtime 25, pointer/launcher 26, runner 57, legacy dispatcher 46, route-smoke 13, install lifecycle 17 passed/2 skipped, and managed-runtime units 16 passed; real-uv checks unavailable in sandbox because `~/.cache/uv` is read-only | `d5584e7` | complete |
| `S5` | Representative nested v6 route proves exact lookup, authorization, confinement, no enumeration or writes, scale-independent work, and source-scope latency; CLI/package imports are lazy | Four isolated pilot performance gates passed: warm resolver median below 50 ms, fresh-checkout CLI median below 100 ms and p95 below 150 ms, unchanged reads/probes after 500 unrelated modules, and forbidden scan/subprocess/write operations absent. Numeric gates are full-only and receive the complete runner lease so pooled correctness work cannot corrupt wall-clock evidence | `07a82f7` | complete |
| `S6` | All live code-bearing modules, behavioral sources, generated skill contracts, caller IDs, active schemas, archived v5 references, standards blueprints, and the runtime dependency manifest are reconciled to v6; target-only semantic-integration, recurring jobs configuration, and shared standard-query architecture are retained | Candidate dispatcher synchronization and read-only check succeeded; migrated inventory contains 221 unique v6 nodes; focused schema/inventory/tooling checks reached 136 passed before the final archived-v5 fixture correction and both affected template nodes then passed; complete route smoke passed 14 tests | `6766e69` | complete |
