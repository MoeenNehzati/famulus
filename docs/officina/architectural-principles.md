...
Readers new to Officina should begin with the [Overview](README.md) and
[Getting Started](getting-started.md). This document is the normative layer;
the mechanism guides explain how the principles are realized.

The principles form four parts:

1. **Boundaries and descriptions (§§1–3):** choose node boundaries, ownership,
   gateways, blueprints, and contracts.
2. **Interaction and graph authority (§§4–6):** declare access, resolution,
   dependencies, and the authority of authored and generated artifacts.
3. **Conformance and assurance (§§7–8):** combine mechanical checks with
   semantic review, certification, and drift.
4. **Evolution and limits (§§9–11):** preserve compatibility, state the limits
   of assurance, and keep principles above implementation detail.

Officina governs the continuous development of systems whose behavior spans
model-interpreted instructions, machine-executable code, and structured
machine-readable contracts. It seeks reliability and maintainability by
dividing such systems into cohesive parts, encapsulating those parts, and
making their relationships explicit.

Conventional languages and toolchains make many code relationships visible and
mechanically checkable through source organization, imports, packages,
interfaces, types, and related tooling. They do not eliminate hidden coupling,
and they do not automatically cover relationships expressed through
human-language instructions. Officina therefore supplies explicit logical
boundaries and graph relationships across every form of behavior in the
repository.

The governing requirements are:

1. Behavior that can evolve independently is separated, when useful, into
   explicit nodes with canonical ownership.
2. Dependencies, interfaces, access, authority, and effects are declared rather
   than inferred from physical proximity or prose convention.
3. Each rule or operation uses the least interpretive representation that can
   express it faithfully: deterministic code where mechanical decision is
   possible, structured data where formal representation is sufficient, and
   free-form instructions for the semantic remainder.
4. Machine-readable blueprints expose the repository graph to schemas,
   validators, authorization machinery, and generated views.
5. Mechanical checks are augmented by retained human- or model-assisted
   certification evidence when semantic judgment is necessary; relevant drift
   invalidates that evidence.

These mechanisms cannot make every semantic property mechanically decidable.
They make more relationships explicit and checkable, reduce the opportunity for
undeclared coupling to accumulate, and retain assurance only for the exact
state supported by current evidence. The sections below state the normative
principles; implementation details belong to the linked technical references.
...
These rules apply to machine-executable code, human-language instructions,
schemas, configuration, and every other artifact that participates in
behavior.
...
When structured data crosses a boundary, its shape must use a
machine-checkable contract whenever that contract can express the boundary
faithfully. JSON Schema is one such contract; narrower declared types and
invocation grammars remain appropriate when they are adequate. The
[Schemas guide](schema.md) explains the available forms and their limits.
...
## Mechanism map (nonnormative)

- [Overview](README.md) introduces the problem and documentation routes.
- [Getting Started](getting-started.md) follows one node profile end to end.
- [Blueprints](blueprints.md) explains the descriptive face of a node.
- [Dispatcher](dispatcher.md) explains resolution, authorization, and execution.
- [Schemas](schema.md) explains machine-checkable boundary contracts.
- [Certification and Drift](certification_and_drift.md) explains retained
  assurance and invalidation.
- [Standards](standards.md) explains how semantic policy is represented and
  queried.
