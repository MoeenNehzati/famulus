...
8. Exclusively create one timestamped credential descriptor, then store its
   refresh token under the descriptor's unique secret reference. If secret
   storage fails, remove only that newly created descriptor; never replace an
   older descriptor.
...
5. The exact loopback callback is validated. The command exchanges the code,
   identifies the account, creates the credential file, binds its absolute path
   through the coordinator, and returns one JSON result.
...
This read preflight cannot prove a later write will succeed; a write failure
removes the newly created descriptor without changing an older authorization.
...
dispatcher --caller-skill connect-google \
  ...
  --services drive,calendar --home /absolute/home --no-open-browser
```
...
`credential_publish_failed`, or `internal_error`. The current descriptor
publisher has no completion-warning variants.
...
The private authorization source emits one success JSON object containing only
`schema_version` (the integer `1`), `account`, `subject`, `credential_file`,
`requested_services`, `granted_services`, and `denied_services`, followed by a
newline. The public coordinator adds per-service binding state and `complete`.
Private authorization failure stdout is empty; coordinator failures retain the
aggregate structured `complete: false` machine-result contract.

## Credential Publication And Compatibility

Each successful authorization creates a new immutable descriptor beneath the
connect-google credentials directory. Its timestamp-and-random filename stem
also identifies the refresh-token secret. The descriptor records the account,
stable Google subject, selected services actually granted, granted scopes,
pinned token endpoint, and opaque client/refresh secret references; it contains
no OAuth secret or access token.

Publication is intentionally small:

1. Exclusively create the new descriptor so an existing file is never replaced.
2. Store the refresh token under the descriptor's unique secret reference.
3. If secret storage fails, remove that new descriptor and return a stable
   publication failure.
4. Return the absolute `credential_file`; the coordinator passes that same path
   to each granted service's service-owned binder.

Each service verifies a live provider call before recording only the descriptor
path it owns. Partial binding is retained and reported; retries use
the same descriptor without another consent flow. Legacy registry loading and
refresh remain read-only compatibility paths. No supported connection workflow
creates or binds a legacy registry identifier.
...
- Assert private authorization success stdout is exactly one JSON object and its
  failure stdout is empty; assert coordinator failures return structured
  `complete: false` JSON.
...
- Fault-inject exclusive descriptor creation and refresh-secret storage. A
  secret-store failure removes only the new descriptor; existing files remain.
- Prove descriptor filenames and refresh-secret references are unique,
  timestamped, and non-overwriting under collisions and concurrent attempts.
- Load legacy registry records only through compatibility reads; no supported
  authorization or binding path writes a registry record.
...
7. Descriptor publication never replaces an older file, cleans up a descriptor
   whose secret write fails, and leaves legacy credentials readable.
