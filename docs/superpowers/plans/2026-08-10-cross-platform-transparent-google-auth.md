...
publishes timestamped credential files for service-owned binding.
...
- Create a new timestamped `credential_file` for every authorization; retain
  legacy registry loading only as read-only compatibility.
...
### Task 2: Timestamped credential-file publication safety

**Files:**

- Modify: `src/officina/common/google_credentials.py`
- Modify: `tests/test_officina_google_credential_files.py`
- Modify: `tests/test_officina_google_credentials.py`
- Modify: `src/officina/common/blueprints/google-credentials.yaml`

**Interfaces:**

- `create_credential_file(...) -> GoogleCredentialFile` exclusively creates a
  timestamped descriptor and stores its refresh token under a filename-derived
  secret reference.
- `load_credential_file(...)` fully validates descriptor shape, filename,
  scopes, services, and secret references.
- `refresh_access_token_from_file(...)` reads only the descriptor's recorded
  opaque secret references and contacts the pinned Google token endpoint.
- Legacy registry load/refresh functions remain compatibility reads but are not
  advertised or used by supported connection workflows.

- [ ] **Step 1: Add descriptor creation, collision, cleanup, and refresh tests.**

  Cover unique timestamped files, exclusive creation, malformed descriptors,
  missing secrets, custom client-secret references, scope enforcement, and
  secret-store failure cleanup without replacing an existing descriptor.

- [ ] **Step 2: Implement and verify the descriptor boundary.**

  ```text
  pytest -q tests/test_officina_google_credential_files.py
  ```

  Expected: descriptor operations pass without any supported registry write.

- [ ] **Step 3: Preserve legacy read compatibility and rerun common tests.**

  Keep existing registry load/refresh behavior available to legacy callers,
  while ensuring `create_credential_file` and `refresh_access_token_from_file`
  never create or bind a registry identifier. Run:

  ```text
  pytest -q tests/test_officina_google_credentials.py
  ```

  Expected: PASS, including legacy reads, pinned endpoints, and client rotation.
...
### Task 5: End-to-end coordinator authorization and binding
...
- The private `authorize_services(..., callback_port=0, browser_enabled=True,
  diagnostic_stream=sys.stderr, ...) -> AuthorizationResult` returns an
  absolute `credential_file`.
- The public `connect-services` coordinator exposes `--no-open-browser` and
  `--callback-port`, forwards them to authorization, and binds the returned path.
- Success stdout preserves `schema_version: 1`; errors return nonzero with one
  terminal OAuth error record and no secret-bearing output.
...
- [ ] **Step 1: Add failing orchestration tests.**
  ...
  subset; account mismatch publishes nothing; success returns an absolute
  `credential_file`; every failure maps to the design's exact phase and code.
  ...
  Callback, token, UserInfo, account, descriptor creation, and secret-store
  boundaries raise or map to this type by exception class and explicit branch;
  no message matching is allowed.
...
- The private authorize source and public `connect-services` process binding
  permit `--no-open-browser` and `--callback-port`.
- Interaction is exactly `mode: interactive`, `channel: tty`,
  `unattended_outcome: failed`.
- The private authorizer declares user-private stderr JSONL and success-only
  stdout; the public coordinator retains structured complete/incomplete/failure
  stdout.

- [ ] **Step 1: Add a failing blueprint consumer test.**

  Load the compiled interface and assert its interaction, allowed flags,
  stdout/error cardinality, pinned endpoints, user-private stderr,
  descriptor-publication and cleanup wording, private authorization failures, and public
  coordinator incomplete/failure outcomes. The break caught is a caller
  receiving a stale executable contract, not a source-text change.
