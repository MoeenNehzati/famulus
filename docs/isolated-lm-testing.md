...
diagnostic goes to standard error. Only when the `kvm:read-write` check reports
...
The launch fields are null/empty while `prepared`, then persisted before QEMU
starts. Lifecycle becomes `running` only after QEMU reports launch success and
`ready` only after SSH and cloud-init complete. Launch failure records
`launch-failed`; shutdown records `stopped` only after the exact recorded QEMU
process is absent. Serial logs, manifests, source evidence, and the overlay are
...
- Download or verification failure: no successful source-image manifest is
  emitted. Temporary downloads are removed and an existing destination is
  replaced only after the new bytes pass the applicable checks. Correct the
  network/package/trust failure and rerun `prepare-image` with the same state
  root. Do not hand-edit `source-image.json` or bypass `gpgv`.
...
- QEMU launch failure: `run.json` retains launch facts and reports
  `launch-failed`; inspect the JSON diagnostic and `serial.log`. Use `stop-run`
  with the recorded key to confirm bounded cleanup, then prepare a fresh run ID.
...
- Shutdown failure: the manifest retains its prior lifecycle. Inspect the
  ...
  The harness refuses to act when a live PID does not contain both the exact VM
  name and overlay path; investigate that mismatch instead of weakening the
  identity check.
